...
def get_file_name(file_format, dynamic=False):
    if dynamic:
        ...
        # Générer un nom du fichier fixe
...
"""
    Extraire les champs, types et données du fichier de données en tenant compte des commentaires
"""
def parse_data_file(file_path):
    try:
        ...
        for line in lines:
            if SPLIT in line:
                current_field, field_type = line.split(SPLIT)
                fields.append((current_field, field_type))
                data[current_field] = []
            else:
...
"""
    Extraire les exclusions en tenant compte des commentaires
"""
...
def create_database(fields):
    try:
        ...
        # Préparer les champs
        columns = ", ".join([f"{field[0]} {field[1]}" for field in fields])
...
def apply_exclusions(cursor, exclusions):
    for exclusion in exclusions:
        if exclusion and not exclusion.startswith(COMMENT):
...
def main():
    try:
        ...
        # Analyser le fichier de données et extraire les champs, types puis données correspondantes en tenant compte des commentaires
